...
int main()
{
	for(int i=0; i<ARRAY_SIZE; ++i) arr1[i] = 10;
	
	//for(int i=0; i<ARRAY_SIZE; ++i) printf("val : %d \n", arr1[i]);
	for(int i=0; i< ARRAY_SIZE; ++i) std::cout<<(int)arr1[i]<<endl;
	
	vadd_variations(arr1, arr3, ARRAY_SIZE);
	for(int i=0; i<ARRAY_SIZE; ++i) printf("val : %d \n", arr3[i]);
